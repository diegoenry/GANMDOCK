...
int main( int argc , char *argv[] ) {
  ...
  strcpy( complex_file_name,   "complex.parsed" );
  strcpy( contact_file_name,   "contact.list"   );
  strcpy( interface_file_name, "interface.pdb"  );
...
void print_usage() {
  ...
  printf("Options:\n");
  printf("\t-s  <pdbfile of the complex input> default \"complex.parsed\"\n");
  printf("\t-r  <receptor chain IDs> required\n");
  printf("\t-l  <ligand chain IDs> required\n");
  printf("\t-c  <contact output file> default \"contact.list\"\n");
  printf("\t-ca output only coordinates of ca atoms\n");
  printf("\t-i  <interface pdb file> default \"interface.pdb\"\n");
